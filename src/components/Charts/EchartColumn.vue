...
import { BASIC_COLUMN_OPTIONS, EMPTY_COLUMN_OPTIONS } from './default_options'
export default {
  name: 'EchartColumn',
    props: {
    containerId: {
  ...
  methods: {
    ...
    mergeOptions() {
      ...
      return merge(
        {},
        BASIC_COLUMN_OPTIONS,
        {
          dataset: {
            source: _this.optionsData.sourceList
          }
        }
      )
    },
    ...
    updateChartView() {
      if (!this.$_basicChart) return
      const mergeOptions = isEmpty(this.optionsData.sourceList) ? EMPTY_COLUMN_OPTIONS : this.mergeOptions()
